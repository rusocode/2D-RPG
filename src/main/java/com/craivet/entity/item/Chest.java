...
public class Chest extends Item {
	...
	public void interact() {
		...
		if (!opened) {
			...
			image = image2;
			opened = true;
			if (game.player.inventory.size() == MAX_INVENTORY_SIZE)
			...
			sb.append(empty ? "It's empty" : (game.player.inventory.size() == MAX_INVENTORY_SIZE) ? "You cannot carry any more!" : "You obtain the " + loot.name + "!");
			if (!empty && game.player.inventory.size() != MAX_INVENTORY_SIZE) {
				game.player.inventory.add(loot);
				empty = true;
			}
		}
